...
float halfT;                   // 采样周期的一半，用于求解四元数微分方程时计算角增量
struct ANGLE IMUupdate(float ax, float ay, float az, float gx, float gy, float gz,short frq) //g表陀螺仪，a表加计
{
...
struct COORD transform(vector<double>q4,double x,double y,double z){
    double q0 = q4[0],q1 = q4[1],q2 = q4[2],q3 = q4[3];

    double A00 = 1-2*q2*q2-2*q3*q3;
    double A01 = 2*q1*q2+2*q0*q3;
    double A02 = 2*q1*q3-2*q0*q2;
    double A10 = 2*q1*q2-2*q0*q3;
    double A11 = 1-2*q1*q1-2*q3*q3;
    double A12 = 2*q2*q3+2*q0*q1;
    double A20 = 2*q1*q3+2*q0*q2;
    double A21 = 2*q2*q3-2*q0*q1;
    double A22 = 1-2*q1*q1-2*q2*q2;
